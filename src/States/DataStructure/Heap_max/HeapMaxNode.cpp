...
HeapMaxNode::HeapMaxNode(float x, float y, float scale_x, float scale_y, int _key, int idx_array, sf::Font *_font, std::map<std::string, sf::Color> *Colors)
{
    this->Colors = Colors;
...
{   
    ...
    array_label.clear();
    for (int i = 0; i < numChildHeapMax; ++i) array_next[i].clear(); 
    array_key.clear();
    array_pos.clear();
    ...
    idx_outlineColor = -1;
    idx_label = -1;
    for (int i = 0; i < numChildHeapMax; ++i) idx_next[i] = -1;
    idx_key = -1;
    idx_pos = -1;
...
//Next
void HeapMaxNode::updateNext(int next_index)
{
...
void HeapMaxNode::newNext(HeapMaxNode *new_next, int next_index)
{
...
void HeapMaxNode::prevNext(int next_index)
{
    this->idx_next[next_index]--;
    this->updateNext(next_index);
}

void HeapMaxNode::nextNext(int next_index)
{
    this->idx_next[next_index]++;
    this->updateNext(next_index);
}
...
//key
void HeapMaxNode::updateKey()
{
    this->key = array_key[idx_key];
}

void HeapMaxNode::newKey(int new_key)
{
...
void HeapMaxNode::prevKey()
{   
    this->idx_key--;
    this->updateKey();
}

void HeapMaxNode::nextKey()
{
    this->idx_key++;
    this->updateKey();
}
...
//Color
void HeapMaxNode::setNormalColor()
{
...
void HeapMaxNode::newColor(std::string fillColor, std::string outlineColor)
{
...
void HeapMaxNode::newLabel(std::string new_label)
{
...
void HeapMaxNode::prevLabel()
{
    this->idx_label--;
    this->updateLabel();
}

void HeapMaxNode::nextLabel()
{
    this->idx_label++;
    this->updateLabel();
}
...
void HeapMaxNode::newPos(sf::Vector2f new_pos)
{
...
void HeapMaxNode::prevPos()
{
    this->idx_pos--;
    this->updatePos();
}

void HeapMaxNode::nextPos()
{
    this->idx_pos++;
    this->updatePos();
}
...
//Render
void HeapMaxNode::renderAnimation()
{
}

void HeapMaxNode::render(sf::RenderTarget *target)
{
    this->updateText();
    this->updateLabel();
...
void RecalTreeAmountLeftRight(HeapMaxNode *root, int direction, bool isRoot)
{   
    if (root == nullptr) return;
    if (isRoot) root->depthHeapMax = 0;

    if (root->next[0]) root->next[0]->depthHeapMax = root->depthHeapMax + 1;
    if (root->next[1]) root->next[1]->depthHeapMax = root->depthHeapMax + 1;
...
void RecalTreePosition(HeapMaxNode *root, float start_x, float start_y, float distance_x, float distance_y, int direction)
{
    if (root == nullptr) return;

    if (root->depthHeapMax == 0) {
    ...
    root->newPos(sf::Vector2f(start_x + direction * distance_x * (root->amountLR[std::max(-direction, 0)] + 1 /* root */),
                              start_y + distance_y * root->depthHeapMax));
...
{   
    ...
    for (int i = 0; i < numChildHeapMax; ++i) {
        ...
        if (root->next[i])
        {
            makeArrowHeapMax(&root->shape, &root->next[i]->shape, &root->arrow[i]);
            root->showArrow[i] = 1;
        }
    }

    // std::cout<<"\nkey,depth,L,R="<<root->key<<' '<<root->depthHeapMax<<' ';
...
sf::Vector2f getPositionNode(HeapMaxNode *root, HeapMaxNode *node, float start_x, float start_y, float distance_x, float distance_y)
{
    ...
    return sf::Vector2f(start_x + direction * distance_x * (node->amountLR[std::max(-direction, 0)] + 1 /* root */),
                        start_y + distance_y * node->depthHeapMax);
}

void updateBalanceFactor(HeapMaxNode *root)
{
    if (root == NULL) return;
    // root->balanceFactor = heightHeapMax(root->next[0]) - heightHeapMax(root->next[1]);
}
