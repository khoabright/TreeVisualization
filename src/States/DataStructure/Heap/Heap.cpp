...
Heap::Heap(sf::RenderWindow *window, std::map<std::string, int> *supportedKeys, std::stack<State *> *states)
...
Heap::~Heap()
{
    ...
    delete (this->animationHeap);
}

void Heap::DeleteNodePointers()
{
...
/* Init */
void Heap::initTriggerFunction()
{
    auto InstructionButtons = [&]()
    {
        this->triggerFunction["Theme"] = std::bind(&Heap::button_theme, this);
        this->triggerFunction["Initialize"] = std::bind(&Heap::button_initialize, this);
        this->triggerFunction["Add"] = std::bind(&Heap::button_add, this);
        this->triggerFunction["Delete"] = std::bind(&Heap::button_delete, this);
        this->triggerFunction["Top"] = std::bind(&Heap::button_top, this);
        this->triggerFunction["Size"] = std::bind(&Heap::button_size, this);
        this->triggerFunction["Quit"] = std::bind(&Heap::button_quit, this);
        this->triggerFunction["SpeedMinus"] = std::bind(&Heap::button_SpeedMinus, this);
        this->triggerFunction["SpeedPlus"] = std::bind(&Heap::button_SpeedPlus, this);
        this->triggerFunction["CodeSwitch"] = std::bind(&Heap::button_CodeSwitch, this);
    };

    auto RunButtons = [&]()
    {
        this->triggerFunction["First"] = std::bind(&Heap::button_first, this);
        this->triggerFunction["Prev"] = std::bind(&Heap::button_prev, this);
        this->triggerFunction["Pause"] = std::bind(&Heap::button_pause, this);
        this->triggerFunction["Play"] = std::bind(&Heap::button_play, this);
        this->triggerFunction["Replay"] = std::bind(&Heap::button_replay, this);
        this->triggerFunction["Next"] = std::bind(&Heap::button_next, this);
        this->triggerFunction["Last"] = std::bind(&Heap::button_last, this);
    };
...
void Heap::initAnimation()
{
    this->animation = new AnimationHeap(&this->Colors, (float)(maxSpeed * speedGap) / currentSpeed, codeHighlight);
    this->animationHeap = dynamic_cast<AnimationHeap *>(this->animation);
    assert(animationHeap != nullptr);
    this->animationHeap->animationTime = float(speedGap * maxSpeed) / currentSpeed;
}

void Heap::initHighlightCode()
{
    codeHighlight = new CodeHighlight(scale_x, scale_y, &Colors);
}

void Heap::button_theme()
{
...
void Heap::button_initialize()
{
    ...
    if (this->choosingChildButton == "2Random")
    {
        ...
        {
            codeHighlight->introText.setString("Random HeapTree with " + std::to_string(valueFirst) + " node" + (valueFirst > 1 ? "s" : ""));
        ...
        for (int i = 0; i < this->valueFirst; ++i)
        {
            ...
            this->animationHeap->newInstruction(this->root, arrNode, numberNode, this->start_x, this->start_y, this->nodeDistanceX, this->nodeDistanceY);
            addHighlightCodes();
            insertHeapNode(newValue);
        }
        // this->animationHeap->updateNodePosition(this->root, this->start_x, this->start_y, this->nodeDistanceX, this->nodeDistanceY);
        return;
    }
    ...
    if (this->choosingChildButton == "3Load File")
    {
        ...
        std::ifstream inp;
        inp.open("InputFiles/inputHeap.txt");
        int newValue = 0;
        while (inp >> newValue)
        {
            this->animationHeap->newInstruction(this->root, arrNode, numberNode, this->start_x, this->start_y, this->nodeDistanceX, this->nodeDistanceY);
        ...
        // this->animationHeap->updateNodePosition(this->head, this->tail, this->start_x, this->start_y, this->nodeDistanceX);
        return;
    }
}

void Heap::button_add()
{
...
void Heap::button_delete()
{
    this->operation_delete();
    return;
}

void Heap::button_top()
{
    operation_top();
    return;
}

void Heap::button_size()
{
    operation_size();
    return;
}

void Heap::button_quit()
{
    this->endState();
}
...
void Heap::buildHeapTree(HeapNode *cur)
{      
    Nodes.push_back(cur);
    int idx = cur->idxArray;
    arrNode[idx] = cur;
    if (2 * idx <= maxNode) {
        cur->next[0] = new HeapNode(0, 0, scale_x, scale_y, 2 * idx, 2 * idx, &font, &Colors);
        buildHeapTree(cur->next[0]);
    }
    if (2 * idx + 1 <= maxNode) {
        cur->next[1] = new HeapNode(0, 0, scale_x, scale_y, 2 * idx + 1, 2 * idx + 1, &font, &Colors);
        buildHeapTree(cur->next[1]);
    }
}

HeapNode *Heap::newHeapNode(int key, HeapNode *parentNode)
{
    ...
    HeapNode *newNode = new HeapNode(posX, posY, scale_x, scale_y, key, 1, &font, &Colors);
    newNode->key = key;
    newNode->heightHeap = 1;
    this->Nodes.push_back(newNode);
    this->animationHeap->instructions.push_back({[this, newNode]()
                                                { this->animationHeap->showNode(newNode, "", this->numberNode, {1}); },
                                                [this, parentNode]()
                                                { this->animationHeap->highlightCurrentNode(parentNode, "normal", {5}); }});
    waitAnimation();

    return (newNode);
}

void Heap::insertHeapNode(int key)
{   
    ++numberNode;
    ...
    std::vector<std::function<void()>> temp_vec = {[this]()
                                                   { this->animationHeap->showNode(arrNode[numberNode], "", numberNode, {1}); }};
    
    if (numberNode > 1)
        temp_vec.push_back([this]()
                           { this->animationHeap->connectNodes(arrNode[numberNode / 2], arrNode[numberNode], numberNode % 2, {1}); });
    this->animationHeap->instructions.push_back(temp_vec);
    waitAnimation();
    ...
    {   
        bool swapped = 0;
        this->animationHeap->instructions.push_back({[this, curIdx]()
                                                     { this->animationHeap->highlightCurrentNode(arrNode[curIdx], "reachColor", {5}); },
                                                     [this, curIdx]()
                                                     { this->animationHeap->highlightCurrentNode(arrNode[curIdx / 2], "reachColor", {5}); }});
        waitAnimation();
        ...
        if (arrNode[curIdx]->key < arrNode[curIdx / 2]->key) {
            ...
            this->animationHeap->instructions.push_back({[this, curIdx, val2]()
                                                         { this->animationHeap->changeNodeValue(arrNode[curIdx], val2, {5}); },
                                                         [this, curIdx, val1]()
                                                         { this->animationHeap->changeNodeValue(arrNode[curIdx / 2], val1, {5}); }});
            waitAnimation();
        }

        this->animationHeap->instructions.push_back({[this, curIdx]()
                                                        { this->animationHeap->highlightCurrentNode(arrNode[curIdx], "normal", {5}); },
                                                        [this, curIdx]()
                                                        { this->animationHeap->highlightCurrentNode(arrNode[curIdx / 2], "normal", {5}); }});
        waitAnimation();
        if (!swapped) break;
...
HeapNode *Heap::deleteHeapNode()
{
    int val1 = arrNode[1]->key;
    int val2 = arrNode[numberNode]->key;
    this->animationHeap->instructions.push_back({[this, val2]()
                                                 { this->animationHeap->changeNodeValue(arrNode[1], val2, {5}); }});
    waitAnimation();

    this->animationHeap->instructions.push_back({[this, sz = this->numberNode]()
                                                 { this->animationHeap->hideNode(arrNode[sz], arrNode[sz / 2], sz % 2, numberNode, {5}); }});
    waitAnimation();
    ...
    while (curIdx * 2 <= numberNode) {
        if (curIdx * 2 + 1 <= numberNode) // compare only when exist 2 children
        {
            this->animationHeap->instructions.push_back({[this, curIdx]()
                                                         { this->animationHeap->highlightCurrentNode(arrNode[curIdx * 2], "reachColor", {5}); },
                                                         [this, curIdx]()
                                                         { this->animationHeap->highlightCurrentNode(arrNode[curIdx * 2 + 1], "reachColor", {5}); }});
            waitAnimation();
        }
        ...
        this->animationHeap->instructions.push_back({[this, curIdx]()
                                                     { this->animationHeap->highlightCurrentNode(arrNode[curIdx], "reachColor", {5}); },
                                                     [this, curIdx, child]()
                                                     { this->animationHeap->highlightCurrentNode(arrNode[curIdx * 2 + 1 - child], "normal", {5}); },
                                                     [this, curIdx, child]()
                                                     { this->animationHeap->highlightCurrentNode(arrNode[curIdx * 2 + child], "reachColor", {5}); }});
        ...
        std::vector<std::function<void()>> temp_vec = {[this, curIdx]()
                                                     { this->animationHeap->highlightCurrentNode(arrNode[curIdx], "normal", {5}); },
                                                     [this, curIdx, child]()
                                                     { this->animationHeap->highlightCurrentNode(arrNode[curIdx * 2 + child], "normal", {5}); }};
        ...
        if (arrNode[curIdx]->key > arrNode[curIdx * 2 + child]->key) {
            ...
            temp_vec.push_back([this, curIdx, child, val1]()
                               { this->animationHeap->changeNodeValue(arrNode[curIdx * 2 + child], val1, {5}); });
        }
        this->animationHeap->instructions.push_back(temp_vec);
        waitAnimation();
...
void Heap::operation_add(int nodeValue)
{
    ...
    insertHeapNode(nodeValue);

    endOperation();
    return;
}

void Heap::operation_delete()
{
    this->prepareNewInstruction();
    this->button_play();
    ...
    // this->animationHeap->instructions.push_back({[this]()
    //                                             { this->animationHeap->Relayout((numberNode == 0), root->next[0], start_x, start_y, nodeDistanceX, nodeDistanceY, {5}); }});
...
void Heap::operation_top()
{
    ...
    this->animationHeap->instructions.push_back({[this]()
                                                     { this->animationHeap->highlightCurrentNode(root, "reachColor", {}); }});

    return;
}

void Heap::operation_size()
{
    this->prepareNewInstruction();
    this->button_play();
...
void Heap::prepareNewInstruction()
{
    // std::cout << "Prepare new\n";
    this->animation->finishStep();
    bool trash = 0;
    this->animationHeap->last(trash, &this->stepText);
    this->animationHeap->newInstruction(this->root, arrNode, numberNode, this->start_x, this->start_y, this->nodeDistanceX, this->nodeDistanceY);
    this->animationHeap->animationTime = 0.001; /* Do all instruction quickly first, then activate replay mode */
    this->codeHighlight->reset();
}

/* Update and Render */
void Heap::updateButtons()
{
...
void Heap::updateSFMLEvents(const float &dt, sf::Event &event)
{
    // for Buttons
    for (auto &it : this->buttons)
    {
    ...
    if (this->choosingButton != "" && this->choosingChildButton != "")
    {
            ...
            inputFields[choosingButton].find(choosingChildButton) != inputFields[choosingButton].end())
        {
            auto readingInput = [&]()
            {
                ...
                // {
                //     this->inputWarning.setString("Sorry, the animationHeap is running...");
                //     return;
                // }
...
void Heap::renderButtons(sf::RenderTarget *target)
{
...
void Heap::renderNode(sf::RenderTarget *target)
{
    for (HeapNode *node : this->Nodes)
    {
        node->render(target);
    }
}

void Heap::renderAnimation()
{
    ...
    {
        this->animationHeap->play(this->doneAnimation, &this->stepText);
        if (this->doneAnimation)
        {
            enable_replayButton();
        }
    ...
    {
        this->animationHeap->pause(this->doneAnimation, &this->stepText);
        return;
    }

    if (this->runState == "prev")
    {
        this->animationHeap->prev(this->doneAnimation, &this->stepText);
        return;
    }

    if (this->runState == "next")
    {
        this->animationHeap->next(this->doneAnimation, &this->stepText);
        if (this->doneAnimation && this->animationHeap->curIndex == (int)this->animationHeap->instructions.size())
        {
            enable_replayButton();
        }
        return;
    ...
    {
        this->animationHeap->first(this->doneAnimation, &this->stepText);
        return;
    }

    if (this->runState == "last")
    {
        this->animationHeap->last(this->doneAnimation, &this->stepText);
        if (this->doneAnimation)
        {
            enable_replayButton();
        }
...
void Heap::renderHighlightCode(sf::RenderTarget *target)
{
    codeHighlight->render(target);
}

void Heap::render(sf::RenderTarget *target)
{
    if (!target)
    {
        target = this->window;
...
{   
    ...
    /* Init Tree */
    this->root = new HeapNode(0, 0, scale_x, scale_y, 1, 1, &font, &Colors);
    HeapNode* rubbish = new HeapNode(0, 0, scale_x, scale_y, -1, 0, &font, &Colors);
