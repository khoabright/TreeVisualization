...
AnimationHeap::AnimationHeap(std::map<std::string, sf::Color> *Colors, float animationTime, CodeHighlight *codeHighlight)
...
void AnimationHeap::addCodeLines(std::vector<int> codeLines)
{
...
// void AnimationHeap::changeRoot(int &idx_root, std::vector<HeapNode *> array_root, HeapNode* new_root, HeapNode* root, std::vector<int> codeLines)
// {
...
void AnimationHeap::showNode(HeapNode *targetNode, std::string targetLabel, int &numberNode, std::vector<int> codeLines) 
{   
    /* For newNode only */
...
void AnimationHeap::hideNode(HeapNode *targetNode, HeapNode *parentNode, int next_index, int &numberNode, std::vector<int> codeLines)
{   
...
void AnimationHeap::changeNodeLabel(HeapNode *targetNode, std::string targetLabel, std::vector<int> codeLines)
{   
    if (!this->startStep[stepChildIndex]) return;
    this->startStep[stepChildIndex] = 0;
...
void AnimationHeap::changeNodeValue(HeapNode *targetNode, int newValue, std::vector<int> codeLines)
{   
    if (!this->startStep[stepChildIndex]) return;
    this->startStep[stepChildIndex] = 0;
...
void AnimationHeap::connectNodes(HeapNode *targetNode, HeapNode *nextNode, int next_index, std::vector<int> codeLines)
{   
    /* Don't waste time to make arrow */
    // this->dt = this->animationTime;

    //In heap, all arrow are set beforehand, only adjust showArrow
    if (this->reverse && this->doneStep)
    {   
...
void AnimationHeap::highlightCurrentNode(HeapNode *targetNode, std::string targetColor, std::vector<int> codeLines)
{   
    if (!this->startStep[stepChildIndex]) return;
    this->startStep[stepChildIndex] = 0;
...
void AnimationHeap::moveNode(HeapNode *targetNode, float x1, float y1)
{   
    if (this->reverse && this->doneStep)
    {   
        targetNode->prevPos();
...
void AnimationHeap::Relayout(bool emptyList, HeapNode *root, float start_x, float start_y, float distance_x, float distance_y, std::vector<int> codeLines)
{       
    if (emptyList) {
        if (!this->startStep[stepChildIndex]) return;
        this->startStep[stepChildIndex] = 0;
    ...
    std::queue<std::pair<HeapNode* , int>> Queue; // second is 1 or -1 represent right/left to the root
    Queue.push({root, 0});
    ...
    while (!Queue.empty()) {
        ...
        //1->0   -1->1
        moveNode(curNode, start_x + direction * distance_x * (curNode->amountLR[std::max(-direction, 0)] + 1 /* root */), start_y + distance_y * curNode->depthHeap);
    }

    /* Update position of arrows */
    std::queue<HeapNode*> q; // node only
    q.push(root);
...
//Reset
void AnimationHeap::updateNodePosition(HeapNode *root, std::vector<HeapNode*> arrNode, int numberNode, float start_x, float start_y, float distance_x, float distance_y)
{
    if (root == nullptr)
        return;
...
void AnimationHeap::newInstruction(HeapNode *root, std::vector<HeapNode*> arrNode, int numberNode, float start_x, float start_y, float distance_x, float distance_y)
{   
